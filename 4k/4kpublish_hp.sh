...
log=/var/log/scripts/4kpublish_hp.log
mailList="TO"
mailListTeste="TO_TEST"
URL_APP_JOB_DATA=http://orchestrator.glb-cip.ninja:30452/api/jobs

lock () {
if mkdir /var/lock/4kpublish_hp.lock; then
    sleep 1;
else
    date >> "$log"
...
unlock () {
rm -rf /var/lock/4kpublish_hp.lock
}
